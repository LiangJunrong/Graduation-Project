<template>
  <el-row>
    ...
    <el-col :xs="24" :sm="7" :md="7" :lg="7" :xl="7">
      <el-card class="box-card" :class="{'boyColor':boyUser,'girlColor':girlUser}">
        <div slot="header" class="clearfix">
          <span>个人中心</span>
          <router-link to="/ChangeUserInfo">
            <el-button style="float: right; padding: 3px 3px 0px 0px" type="text">修改信息</el-button>
          </router-link>
          <router-link to="/ChangeUserPassword">
            <el-button style="float: right; padding: 3px 3px 0px 0px" type="text">修改密码</el-button>
          </router-link>
        </div>
        <div class="myInfo">
          <p class="myInfoTitle">你的信息：
            <el-button @click="logOut()" type="danger" round size="small">注销</el-button>
          </p>
          <p>昵称：
            <span class="text-primary">{{userInfo.userName}}</span>
          </p>
          <p>宿舍楼区：
            <span class="text-primary">{{userInfo.userHostelArea}} {{userInfo.userHostelAddress}}</span>
          </p>
          <p>QQ：
            <span class="text-primary">{{userInfo.userQQ}}</span>
          </p>
        </div>
        <div class="myOrganization">
          ...
          <p>
            <router-link :to="{path:'OrgDetail', query:{'orgId':userClubs._id}}" v-for="(userClubs,index) in userInfo.userClubs" v-if="index<3" :key="userClubs.id">
              <el-button type="primary" round>{{userClubs.clubName}}</el-button>
            </router-link>
          </p>
        </div>
        <div class="myGoods">
          ...
          <router-link :to="{path:'GoodDetail', query:{'goodsId':myGoods._id}}" v-for="(myGoods,index) in userInfo.userGoods" :key="myGoods.id"
            v-if="index<3">
            <p>
              {{index+1}}.
              <span class="text-primary">{{myGoods.goodsName}}</span>
            </p>
          </router-link>
        </div>
      </el-card>
  ...
  export default {
    ...
    methods: {
      ...
      getCanvas() {
        ...
        function makeMadeline(camera, isGood, colors, rotation) {
          ...
          [-1, 1].forEach(function (xSide) {
            ...
            if (xSide == 1) {
              ...
              elbowJoint.rotate = Vector3.sanitize({
                z: TAU / 8
              });
            }
            ...
            if (xSide == -1) {
              // bend right leg
              thigh.rotate = Vector3.sanitize({
                x: -TAU / 16 * 3,
                z: TAU / 16
              });
              shin.rotate = Vector3.sanitize({
                x: TAU / 16 * 5
              });
            }
        ...
        makeMadeline(camera, false, badColor, {
          y: TAU / 2
        });
...
<style scoped>
  ...
  .boyColor {
    border: 2px solid rgb(61, 196, 230);
    border-radius: 2em;
